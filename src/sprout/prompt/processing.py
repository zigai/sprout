...
from sprout.prompt.validation import ContextValidatorFn, ValidationResult, ValidatorType
...
def run_validator(
    ...
    for validator in question.validators:
        callable_validator: Callable[..., ValidationResult] = validator
        if validator_accepts_answers(validator):
            valid, message = callable_validator(raw_value, candidate_answers)
        else:
            valid, message = callable_validator(raw_value)
