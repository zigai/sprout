...
def test_registry_translates_write_failure(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    path = tmp_path / "config" / "templates.json"
    ...
    with pytest.raises(SproutRegistryError) as raised:
        TemplateRegistry(path).save(TrustedTemplate(name="demo", source="owner/demo"))

    assert str(raised.value) == f"failed to write template registry {path}."
    assert raised.value.__cause__ is cause


def test_registry_removes_temporary_file_after_write_failure(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    path = tmp_path / "config" / "templates.json"
    ...
    def fail_dump(value: object, file: object, indent: int | None = None) -> None:
        del value, file, indent
        raise cause
...
def test_registry_rejects_invalid_persisted_config(tmp_path: Path, config: object) -> None:
    ...
    with pytest.raises(SproutRegistryError):
...
def test_registry_rejects_malformed_json(tmp_path: Path) -> None:
    ...
    with pytest.raises(SproutRegistryError, match="failed to read template registry"):
        TemplateRegistry(path).entries()
